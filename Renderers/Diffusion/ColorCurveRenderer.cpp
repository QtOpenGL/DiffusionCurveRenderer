...
bool ColorCurveRenderer::init()
{
    initializeOpenGLFunctions();

    mShader = new QOpenGLShaderProgram;

    if (!mShader->addShaderFromSourceFile(QOpenGLShader::Vertex, "Shaders/Diffusion/ColorCurve/VertexShader.vert")
        || !mShader->addShaderFromSourceFile(QOpenGLShader::Fragment, "Shaders/Diffusion/ColorCurve/FragmentShader.frag")
        || !mShader->addShaderFromSourceFile(QOpenGLShader::Geometry, "Shaders/Diffusion/ColorCurve/GeometryShader.geom") || !mShader->link()
        || !mShader->bind()) {
...
void ColorCurveRenderer::render(const QVector<Curve *> &curves, const QMatrix4x4 &projectionMatrix)
{
    mShader->bind();
    mTicks->bind();
